...
APP:必看小说
变量 如
bkxshd38='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd38');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd38Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd38= $.isNode() ? (process.env.bkxshd38 ? process.env.bkxshd38 : "") : ($.getdata('bkxshd38') ? $.getdata('bkxshd38') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd38Arr.push($.getdata('bkxshd38'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd38Arr.push($.getdata(`bkxshd38${i}`))
            }
    console.log(`------------- 共${bkxshd38Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd38Arr.length; i++) {
        if (bkxshd38Arr[i]) {
          bkxshd38 = bkxshd38Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd38 && process.env.bkxshd38.indexOf('\n') > -1) {
            bkxshd38Arr = process.env.bkxshd38.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd38s = [process.env.bkxshd38]
        };
        Object.keys(bkxshd38s).forEach((item) => {
        if (bkxshd38s[item]) {
            bkxshd38Arr.push(bkxshd38s[item])
        }
    })
          console.log(`共${bkxshd38Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd38Arr.length; k++) {
                $.message = ""
                bkxshd38 = bkxshd38Arr[k]
                $.index = k + 1;
