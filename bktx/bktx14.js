...
APP:必看小说
变量 如
bkxshd14='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('必看小说提现14');
var crypto = require("crypto");
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd14Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd14= $.isNode() ? (process.env.bkxshd14 ? process.env.bkxshd14 : "") : ($.getdata('bkxshd14') ? $.getdata('bkxshd14') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd14Arr.push($.getdata('bkxshd14'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd14Arr.push($.getdata(`bkxshd14${i}`))
            }
    console.log(`------------- 共${bkxshd14Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd14Arr.length; i++) {
        if (bkxshd14Arr[i]) {
          bkxshd14 = bkxshd14Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd14 && process.env.bkxshd14.indexOf('\n') > -1) {
            bkxshd14Arr = process.env.bkxshd14.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd14s = [process.env.bkxshd14]
        };
        Object.keys(bkxshd14s).forEach((item) => {
        if (bkxshd14s[item]) {
            bkxshd14Arr.push(bkxshd14s[item])
        }
    })
          console.log(`共${bkxshd14Arr.length}个cookie`)
 for (let n = 0; n < 70;n++) {
	        for (let k = 0; k < bkxshd14Arr.length; k++) {
                $.message = ""
                bkxshd14 = bkxshd14Arr[k]
                $.index = k + 1;
