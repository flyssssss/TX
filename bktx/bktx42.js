...
APP:必看小说
变量 如
bkxshd42='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd42');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd42Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd42= $.isNode() ? (process.env.bkxshd42 ? process.env.bkxshd42 : "") : ($.getdata('bkxshd42') ? $.getdata('bkxshd42') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd42Arr.push($.getdata('bkxshd42'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd42Arr.push($.getdata(`bkxshd42${i}`))
            }
    console.log(`------------- 共${bkxshd42Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd42Arr.length; i++) {
        if (bkxshd42Arr[i]) {
          bkxshd42 = bkxshd42Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd42 && process.env.bkxshd42.indexOf('\n') > -1) {
            bkxshd42Arr = process.env.bkxshd42.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd42s = [process.env.bkxshd42]
        };
        Object.keys(bkxshd42s).forEach((item) => {
        if (bkxshd42s[item]) {
            bkxshd42Arr.push(bkxshd42s[item])
        }
    })
          console.log(`共${bkxshd42Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd42Arr.length; k++) {
                $.message = ""
                bkxshd42 = bkxshd42Arr[k]
                $.index = k + 1;
