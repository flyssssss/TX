...
APP:必看小说
变量 如
bkxshd12='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('必看小说提现12');
var crypto = require("crypto");
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd12Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd12= $.isNode() ? (process.env.bkxshd12 ? process.env.bkxshd12 : "") : ($.getdata('bkxshd12') ? $.getdata('bkxshd12') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd12Arr.push($.getdata('bkxshd12'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd12Arr.push($.getdata(`bkxshd12${i}`))
            }
    console.log(`------------- 共${bkxshd12Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd12Arr.length; i++) {
        if (bkxshd12Arr[i]) {
          bkxshd12 = bkxshd12Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd12 && process.env.bkxshd12.indexOf('\n') > -1) {
            bkxshd12Arr = process.env.bkxshd12.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd12s = [process.env.bkxshd12]
        };
        Object.keys(bkxshd12s).forEach((item) => {
        if (bkxshd12s[item]) {
            bkxshd12Arr.push(bkxshd12s[item])
        }
    })
          console.log(`共${bkxshd12Arr.length}个cookie`)
 for (let n = 0; n < 70;n++) {
	        for (let k = 0; k < bkxshd12Arr.length; k++) {
                $.message = ""
                bkxshd12 = bkxshd12Arr[k]
                $.index = k + 1;
