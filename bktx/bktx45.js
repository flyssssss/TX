...
APP:必看小说
变量 如
bkxshd45='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd45');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd45Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd45= $.isNode() ? (process.env.bkxshd45 ? process.env.bkxshd45 : "") : ($.getdata('bkxshd45') ? $.getdata('bkxshd45') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd45Arr.push($.getdata('bkxshd45'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd45Arr.push($.getdata(`bkxshd45${i}`))
            }
    console.log(`------------- 共${bkxshd45Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd45Arr.length; i++) {
        if (bkxshd45Arr[i]) {
          bkxshd45 = bkxshd45Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd45 && process.env.bkxshd45.indexOf('\n') > -1) {
            bkxshd45Arr = process.env.bkxshd45.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd45s = [process.env.bkxshd45]
        };
        Object.keys(bkxshd45s).forEach((item) => {
        if (bkxshd45s[item]) {
            bkxshd45Arr.push(bkxshd45s[item])
        }
    })
          console.log(`共${bkxshd45Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd45Arr.length; k++) {
                $.message = ""
                bkxshd45 = bkxshd45Arr[k]
                $.index = k + 1;
