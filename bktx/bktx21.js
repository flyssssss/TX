...
APP:必看小说
变量 如
bkxshd21='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('必看小说提现21');
var crypto = require("crypto");
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd21Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd21= $.isNode() ? (process.env.bkxshd21 ? process.env.bkxshd21 : "") : ($.getdata('bkxshd21') ? $.getdata('bkxshd21') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd21Arr.push($.getdata('bkxshd21'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd21Arr.push($.getdata(`bkxshd21${i}`))
            }
    console.log(`------------- 共${bkxshd21Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd21Arr.length; i++) {
        if (bkxshd21Arr[i]) {
          bkxshd21 = bkxshd21Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd21 && process.env.bkxshd21.indexOf('\n') > -1) {
            bkxshd21Arr = process.env.bkxshd21.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd21s = [process.env.bkxshd21]
        };
        Object.keys(bkxshd21s).forEach((item) => {
        if (bkxshd21s[item]) {
            bkxshd21Arr.push(bkxshd21s[item])
        }
    })
          console.log(`共${bkxshd21Arr.length}个cookie`)
 for (let n = 0; n < 70;n++) {
	        for (let k = 0; k < bkxshd21Arr.length; k++) {
                $.message = ""
                bkxshd21 = bkxshd21Arr[k]
                $.index = k + 1;
