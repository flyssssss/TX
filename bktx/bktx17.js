...
APP:必看小说
变量 如
bkxshd17='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('必看小说提现17');
var crypto = require("crypto");
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd17Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd17= $.isNode() ? (process.env.bkxshd17 ? process.env.bkxshd17 : "") : ($.getdata('bkxshd17') ? $.getdata('bkxshd17') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd17Arr.push($.getdata('bkxshd17'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd17Arr.push($.getdata(`bkxshd17${i}`))
            }
    console.log(`------------- 共${bkxshd17Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd17Arr.length; i++) {
        if (bkxshd17Arr[i]) {
          bkxshd17 = bkxshd17Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd17 && process.env.bkxshd17.indexOf('\n') > -1) {
            bkxshd17Arr = process.env.bkxshd17.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd17s = [process.env.bkxshd17]
        };
        Object.keys(bkxshd17s).forEach((item) => {
        if (bkxshd17s[item]) {
            bkxshd17Arr.push(bkxshd17s[item])
        }
    })
          console.log(`共${bkxshd17Arr.length}个cookie`)
 for (let n = 0; n < 70;n++) {
	        for (let k = 0; k < bkxshd17Arr.length; k++) {
                $.message = ""
                bkxshd17 = bkxshd17Arr[k]
                $.index = k + 1;
