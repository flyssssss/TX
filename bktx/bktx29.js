...
APP:必看小说
变量 如
bkxshd29='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd29');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd29Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd29= $.isNode() ? (process.env.bkxshd29 ? process.env.bkxshd29 : "") : ($.getdata('bkxshd29') ? $.getdata('bkxshd29') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd29Arr.push($.getdata('bkxshd29'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd29Arr.push($.getdata(`bkxshd29${i}`))
            }
    console.log(`------------- 共${bkxshd29Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd29Arr.length; i++) {
        if (bkxshd29Arr[i]) {
          bkxshd29 = bkxshd29Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd29 && process.env.bkxshd29.indexOf('\n') > -1) {
            bkxshd29Arr = process.env.bkxshd29.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd29s = [process.env.bkxshd29]
        };
        Object.keys(bkxshd29s).forEach((item) => {
        if (bkxshd29s[item]) {
            bkxshd29Arr.push(bkxshd29s[item])
        }
    })
          console.log(`共${bkxshd29Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd29Arr.length; k++) {
                $.message = ""
                bkxshd29 = bkxshd29Arr[k]
                $.index = k + 1;
