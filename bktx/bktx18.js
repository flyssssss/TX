...
APP:必看小说
变量 如
bkxshd18='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('必看小说提现18');
var crypto = require("crypto");
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd18Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd18= $.isNode() ? (process.env.bkxshd18 ? process.env.bkxshd18 : "") : ($.getdata('bkxshd18') ? $.getdata('bkxshd18') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd18Arr.push($.getdata('bkxshd18'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd18Arr.push($.getdata(`bkxshd18${i}`))
            }
    console.log(`------------- 共${bkxshd18Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd18Arr.length; i++) {
        if (bkxshd18Arr[i]) {
          bkxshd18 = bkxshd18Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd18 && process.env.bkxshd18.indexOf('\n') > -1) {
            bkxshd18Arr = process.env.bkxshd18.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd18s = [process.env.bkxshd18]
        };
        Object.keys(bkxshd18s).forEach((item) => {
        if (bkxshd18s[item]) {
            bkxshd18Arr.push(bkxshd18s[item])
        }
    })
          console.log(`共${bkxshd18Arr.length}个cookie`)
 for (let n = 0; n < 70;n++) {
	        for (let k = 0; k < bkxshd18Arr.length; k++) {
                $.message = ""
                bkxshd18 = bkxshd18Arr[k]
                $.index = k + 1;
