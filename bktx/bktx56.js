...
APP:必看小说
变量 如
bkxshd56='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd56');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd56Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd56= $.isNode() ? (process.env.bkxshd56 ? process.env.bkxshd56 : "") : ($.getdata('bkxshd56') ? $.getdata('bkxshd56') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd56Arr.push($.getdata('bkxshd56'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd56Arr.push($.getdata(`bkxshd56${i}`))
            }
    console.log(`------------- 共${bkxshd56Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd56Arr.length; i++) {
        if (bkxshd56Arr[i]) {
          bkxshd56 = bkxshd56Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd56 && process.env.bkxshd56.indexOf('\n') > -1) {
            bkxshd56Arr = process.env.bkxshd56.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd56s = [process.env.bkxshd56]
        };
        Object.keys(bkxshd56s).forEach((item) => {
        if (bkxshd56s[item]) {
            bkxshd56Arr.push(bkxshd56s[item])
        }
    })
          console.log(`共${bkxshd56Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd56Arr.length; k++) {
                $.message = ""
                bkxshd56 = bkxshd56Arr[k]
                $.index = k + 1;
