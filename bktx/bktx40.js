...
APP:必看小说
变量 如
bkxshd40='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd40');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd40Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd40= $.isNode() ? (process.env.bkxshd40 ? process.env.bkxshd40 : "") : ($.getdata('bkxshd40') ? $.getdata('bkxshd40') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd40Arr.push($.getdata('bkxshd40'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd40Arr.push($.getdata(`bkxshd40${i}`))
            }
    console.log(`------------- 共${bkxshd40Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd40Arr.length; i++) {
        if (bkxshd40Arr[i]) {
          bkxshd40 = bkxshd40Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd40 && process.env.bkxshd40.indexOf('\n') > -1) {
            bkxshd40Arr = process.env.bkxshd40.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd40s = [process.env.bkxshd40]
        };
        Object.keys(bkxshd40s).forEach((item) => {
        if (bkxshd40s[item]) {
            bkxshd40Arr.push(bkxshd40s[item])
        }
    })
          console.log(`共${bkxshd40Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd40Arr.length; k++) {
                $.message = ""
                bkxshd40 = bkxshd40Arr[k]
                $.index = k + 1;
