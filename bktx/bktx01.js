...
APP:必看小说
变量 如
bkxshd1='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd01');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd1Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd1= $.isNode() ? (process.env.bkxshd1 ? process.env.bkxshd1 : "") : ($.getdata('bkxshd1') ? $.getdata('bkxshd1') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd1Arr.push($.getdata('bkxshd1'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd1Arr.push($.getdata(`bkxshd1${i}`))
            }
    console.log(`------------- 共${bkxshd1Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd1Arr.length; i++) {
        if (bkxshd1Arr[i]) {
          bkxshd1 = bkxshd1Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd1 && process.env.bkxshd1.indexOf('\n') > -1) {
            bkxshd1Arr = process.env.bkxshd1.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd1s = [process.env.bkxshd1]
        };
        Object.keys(bkxshd1s).forEach((item) => {
        if (bkxshd1s[item]) {
            bkxshd1Arr.push(bkxshd1s[item])
        }
    })
          console.log(`共${bkxshd1Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd1Arr.length; k++) {
                $.message = ""
                bkxshd1 = bkxshd1Arr[k]
                $.index = k + 1;
