...
APP:必看小说
变量 如
bkxshd49='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd49');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd49Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd49= $.isNode() ? (process.env.bkxshd49 ? process.env.bkxshd49 : "") : ($.getdata('bkxshd49') ? $.getdata('bkxshd49') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd49Arr.push($.getdata('bkxshd49'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd49Arr.push($.getdata(`bkxshd49${i}`))
            }
    console.log(`------------- 共${bkxshd49Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd49Arr.length; i++) {
        if (bkxshd49Arr[i]) {
          bkxshd49 = bkxshd49Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd49 && process.env.bkxshd49.indexOf('\n') > -1) {
            bkxshd49Arr = process.env.bkxshd49.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd49s = [process.env.bkxshd49]
        };
        Object.keys(bkxshd49s).forEach((item) => {
        if (bkxshd49s[item]) {
            bkxshd49Arr.push(bkxshd49s[item])
        }
    })
          console.log(`共${bkxshd49Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd49Arr.length; k++) {
                $.message = ""
                bkxshd49 = bkxshd49Arr[k]
                $.index = k + 1;
