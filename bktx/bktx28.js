...
APP:必看小说
变量 如
bkxshd28='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd28');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd28Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd28= $.isNode() ? (process.env.bkxshd28 ? process.env.bkxshd28 : "") : ($.getdata('bkxshd28') ? $.getdata('bkxshd28') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd28Arr.push($.getdata('bkxshd28'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd28Arr.push($.getdata(`bkxshd28${i}`))
            }
    console.log(`------------- 共${bkxshd28Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd28Arr.length; i++) {
        if (bkxshd28Arr[i]) {
          bkxshd28 = bkxshd28Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd28 && process.env.bkxshd28.indexOf('\n') > -1) {
            bkxshd28Arr = process.env.bkxshd28.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd28s = [process.env.bkxshd28]
        };
        Object.keys(bkxshd28s).forEach((item) => {
        if (bkxshd28s[item]) {
            bkxshd28Arr.push(bkxshd28s[item])
        }
    })
          console.log(`共${bkxshd28Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd28Arr.length; k++) {
                $.message = ""
                bkxshd28 = bkxshd28Arr[k]
                $.index = k + 1;
