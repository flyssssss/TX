...
APP:必看小说
变量 如
bkxshd32='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd32');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd32Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd32= $.isNode() ? (process.env.bkxshd32 ? process.env.bkxshd32 : "") : ($.getdata('bkxshd32') ? $.getdata('bkxshd32') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd32Arr.push($.getdata('bkxshd32'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd32Arr.push($.getdata(`bkxshd32${i}`))
            }
    console.log(`------------- 共${bkxshd32Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd32Arr.length; i++) {
        if (bkxshd32Arr[i]) {
          bkxshd32 = bkxshd32Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd32 && process.env.bkxshd32.indexOf('\n') > -1) {
            bkxshd32Arr = process.env.bkxshd32.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd32s = [process.env.bkxshd32]
        };
        Object.keys(bkxshd32s).forEach((item) => {
        if (bkxshd32s[item]) {
            bkxshd32Arr.push(bkxshd32s[item])
        }
    })
          console.log(`共${bkxshd32Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd32Arr.length; k++) {
                $.message = ""
                bkxshd32 = bkxshd32Arr[k]
                $.index = k + 1;
