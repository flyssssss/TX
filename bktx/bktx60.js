...
APP:必看小说
变量 如
bkxshd60='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd60');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd60Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd60= $.isNode() ? (process.env.bkxshd60 ? process.env.bkxshd60 : "") : ($.getdata('bkxshd60') ? $.getdata('bkxshd60') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd60Arr.push($.getdata('bkxshd60'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd60Arr.push($.getdata(`bkxshd60${i}`))
            }
    console.log(`------------- 共${bkxshd60Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd60Arr.length; i++) {
        if (bkxshd60Arr[i]) {
          bkxshd60 = bkxshd60Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd60 && process.env.bkxshd60.indexOf('\n') > -1) {
            bkxshd60Arr = process.env.bkxshd60.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd60s = [process.env.bkxshd60]
        };
        Object.keys(bkxshd60s).forEach((item) => {
        if (bkxshd60s[item]) {
            bkxshd60Arr.push(bkxshd60s[item])
        }
    })
          console.log(`共${bkxshd60Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd60Arr.length; k++) {
                $.message = ""
                bkxshd60 = bkxshd60Arr[k]
                $.index = k + 1;
