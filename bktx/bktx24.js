...
APP:必看小说
变量 如
bkxshd24='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd24');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd24Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd24= $.isNode() ? (process.env.bkxshd24 ? process.env.bkxshd24 : "") : ($.getdata('bkxshd24') ? $.getdata('bkxshd24') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd24Arr.push($.getdata('bkxshd24'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd24Arr.push($.getdata(`bkxshd24${i}`))
            }
    console.log(`------------- 共${bkxshd24Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd24Arr.length; i++) {
        if (bkxshd24Arr[i]) {
          bkxshd24 = bkxshd24Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd24 && process.env.bkxshd24.indexOf('\n') > -1) {
            bkxshd24Arr = process.env.bkxshd24.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd24s = [process.env.bkxshd24]
        };
        Object.keys(bkxshd24s).forEach((item) => {
        if (bkxshd24s[item]) {
            bkxshd24Arr.push(bkxshd24s[item])
        }
    })
          console.log(`共${bkxshd24Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd24Arr.length; k++) {
                $.message = ""
                bkxshd24 = bkxshd24Arr[k]
                $.index = k + 1;
