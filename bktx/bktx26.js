...
APP:必看小说
变量 如
bkxshd26='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd26');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd26Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd26= $.isNode() ? (process.env.bkxshd26 ? process.env.bkxshd26 : "") : ($.getdata('bkxshd26') ? $.getdata('bkxshd26') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd26Arr.push($.getdata('bkxshd26'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd26Arr.push($.getdata(`bkxshd26${i}`))
            }
    console.log(`------------- 共${bkxshd26Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd26Arr.length; i++) {
        if (bkxshd26Arr[i]) {
          bkxshd26 = bkxshd26Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd26 && process.env.bkxshd26.indexOf('\n') > -1) {
            bkxshd26Arr = process.env.bkxshd26.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd26s = [process.env.bkxshd26]
        };
        Object.keys(bkxshd26s).forEach((item) => {
        if (bkxshd26s[item]) {
            bkxshd26Arr.push(bkxshd26s[item])
        }
    })
          console.log(`共${bkxshd26Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd26Arr.length; k++) {
                $.message = ""
                bkxshd26 = bkxshd26Arr[k]
                $.index = k + 1;
