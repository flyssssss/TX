...
APP:必看小说
变量 如
bkxshd11='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd11');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd11Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd11= $.isNode() ? (process.env.bkxshd11 ? process.env.bkxshd11 : "") : ($.getdata('bkxshd11') ? $.getdata('bkxshd11') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd11Arr.push($.getdata('bkxshd11'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd11Arr.push($.getdata(`bkxshd11${i}`))
            }
    console.log(`------------- 共${bkxshd11Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd11Arr.length; i++) {
        if (bkxshd11Arr[i]) {
          bkxshd11 = bkxshd11Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd11 && process.env.bkxshd11.indexOf('\n') > -1) {
            bkxshd11Arr = process.env.bkxshd11.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd11s = [process.env.bkxshd11]
        };
        Object.keys(bkxshd11s).forEach((item) => {
        if (bkxshd11s[item]) {
            bkxshd11Arr.push(bkxshd11s[item])
        }
    })
          console.log(`共${bkxshd11Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd11Arr.length; k++) {
                $.message = ""
                bkxshd11 = bkxshd11Arr[k]
                $.index = k + 1;
