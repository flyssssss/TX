...
APP:必看小说
变量 如
bkxshd15='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('必看小说提现15');
var crypto = require("crypto");
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd15Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd15= $.isNode() ? (process.env.bkxshd15 ? process.env.bkxshd15 : "") : ($.getdata('bkxshd15') ? $.getdata('bkxshd15') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd15Arr.push($.getdata('bkxshd15'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd15Arr.push($.getdata(`bkxshd15${i}`))
            }
    console.log(`------------- 共${bkxshd15Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd15Arr.length; i++) {
        if (bkxshd15Arr[i]) {
          bkxshd15 = bkxshd15Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd15 && process.env.bkxshd15.indexOf('\n') > -1) {
            bkxshd15Arr = process.env.bkxshd15.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd15s = [process.env.bkxshd15]
        };
        Object.keys(bkxshd15s).forEach((item) => {
        if (bkxshd15s[item]) {
            bkxshd15Arr.push(bkxshd15s[item])
        }
    })
          console.log(`共${bkxshd15Arr.length}个cookie`)
 for (let n = 0; n < 70;n++) {
	        for (let k = 0; k < bkxshd15Arr.length; k++) {
                $.message = ""
                bkxshd15 = bkxshd15Arr[k]
                $.index = k + 1;
