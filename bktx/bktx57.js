...
APP:必看小说
变量 如
bkxshd57='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd57');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd57Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd57= $.isNode() ? (process.env.bkxshd57 ? process.env.bkxshd57 : "") : ($.getdata('bkxshd57') ? $.getdata('bkxshd57') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd57Arr.push($.getdata('bkxshd57'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd57Arr.push($.getdata(`bkxshd57${i}`))
            }
    console.log(`------------- 共${bkxshd57Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd57Arr.length; i++) {
        if (bkxshd57Arr[i]) {
          bkxshd57 = bkxshd57Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd57 && process.env.bkxshd57.indexOf('\n') > -1) {
            bkxshd57Arr = process.env.bkxshd57.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd57s = [process.env.bkxshd57]
        };
        Object.keys(bkxshd57s).forEach((item) => {
        if (bkxshd57s[item]) {
            bkxshd57Arr.push(bkxshd57s[item])
        }
    })
          console.log(`共${bkxshd57Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd57Arr.length; k++) {
                $.message = ""
                bkxshd57 = bkxshd57Arr[k]
                $.index = k + 1;
