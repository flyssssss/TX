...
APP:必看小说
变量 如
bkxshd48='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd48');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd48Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd48= $.isNode() ? (process.env.bkxshd48 ? process.env.bkxshd48 : "") : ($.getdata('bkxshd48') ? $.getdata('bkxshd48') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd48Arr.push($.getdata('bkxshd48'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd48Arr.push($.getdata(`bkxshd48${i}`))
            }
    console.log(`------------- 共${bkxshd48Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd48Arr.length; i++) {
        if (bkxshd48Arr[i]) {
          bkxshd48 = bkxshd48Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd48 && process.env.bkxshd48.indexOf('\n') > -1) {
            bkxshd48Arr = process.env.bkxshd48.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd48s = [process.env.bkxshd48]
        };
        Object.keys(bkxshd48s).forEach((item) => {
        if (bkxshd48s[item]) {
            bkxshd48Arr.push(bkxshd48s[item])
        }
    })
          console.log(`共${bkxshd48Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd48Arr.length; k++) {
                $.message = ""
                bkxshd48 = bkxshd48Arr[k]
                $.index = k + 1;
