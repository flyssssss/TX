...
APP:必看小说
变量 如
bkxshd54='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd54');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd54Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd54= $.isNode() ? (process.env.bkxshd54 ? process.env.bkxshd54 : "") : ($.getdata('bkxshd54') ? $.getdata('bkxshd54') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd54Arr.push($.getdata('bkxshd54'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd54Arr.push($.getdata(`bkxshd54${i}`))
            }
    console.log(`------------- 共${bkxshd54Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd54Arr.length; i++) {
        if (bkxshd54Arr[i]) {
          bkxshd54 = bkxshd54Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd54 && process.env.bkxshd54.indexOf('\n') > -1) {
            bkxshd54Arr = process.env.bkxshd54.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd54s = [process.env.bkxshd54]
        };
        Object.keys(bkxshd54s).forEach((item) => {
        if (bkxshd54s[item]) {
            bkxshd54Arr.push(bkxshd54s[item])
        }
    })
          console.log(`共${bkxshd54Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd54Arr.length; k++) {
                $.message = ""
                bkxshd54 = bkxshd54Arr[k]
                $.index = k + 1;
