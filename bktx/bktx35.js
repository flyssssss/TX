...
APP:必看小说
变量 如
bkxshd35='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd35');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd35Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd35= $.isNode() ? (process.env.bkxshd35 ? process.env.bkxshd35 : "") : ($.getdata('bkxshd35') ? $.getdata('bkxshd35') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd35Arr.push($.getdata('bkxshd35'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd35Arr.push($.getdata(`bkxshd35${i}`))
            }
    console.log(`------------- 共${bkxshd35Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd35Arr.length; i++) {
        if (bkxshd35Arr[i]) {
          bkxshd35 = bkxshd35Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd35 && process.env.bkxshd35.indexOf('\n') > -1) {
            bkxshd35Arr = process.env.bkxshd35.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd35s = [process.env.bkxshd35]
        };
        Object.keys(bkxshd35s).forEach((item) => {
        if (bkxshd35s[item]) {
            bkxshd35Arr.push(bkxshd35s[item])
        }
    })
          console.log(`共${bkxshd35Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd35Arr.length; k++) {
                $.message = ""
                bkxshd35 = bkxshd35Arr[k]
                $.index = k + 1;
