...
APP:必看小说
变量 如
bkxshd58='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd58');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd58Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd58= $.isNode() ? (process.env.bkxshd58 ? process.env.bkxshd58 : "") : ($.getdata('bkxshd58') ? $.getdata('bkxshd58') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd58Arr.push($.getdata('bkxshd58'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd58Arr.push($.getdata(`bkxshd58${i}`))
            }
    console.log(`------------- 共${bkxshd58Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd58Arr.length; i++) {
        if (bkxshd58Arr[i]) {
          bkxshd58 = bkxshd58Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd58 && process.env.bkxshd58.indexOf('\n') > -1) {
            bkxshd58Arr = process.env.bkxshd58.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd58s = [process.env.bkxshd58]
        };
        Object.keys(bkxshd58s).forEach((item) => {
        if (bkxshd58s[item]) {
            bkxshd58Arr.push(bkxshd58s[item])
        }
    })
          console.log(`共${bkxshd58Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd58Arr.length; k++) {
                $.message = ""
                bkxshd58 = bkxshd58Arr[k]
                $.index = k + 1;
