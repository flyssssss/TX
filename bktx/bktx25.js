...
APP:必看小说
变量 如
bkxshd25='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd25');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd25Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd25= $.isNode() ? (process.env.bkxshd25 ? process.env.bkxshd25 : "") : ($.getdata('bkxshd25') ? $.getdata('bkxshd25') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd25Arr.push($.getdata('bkxshd25'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd25Arr.push($.getdata(`bkxshd25${i}`))
            }
    console.log(`------------- 共${bkxshd25Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd25Arr.length; i++) {
        if (bkxshd25Arr[i]) {
          bkxshd25 = bkxshd25Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd25 && process.env.bkxshd25.indexOf('\n') > -1) {
            bkxshd25Arr = process.env.bkxshd25.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd25s = [process.env.bkxshd25]
        };
        Object.keys(bkxshd25s).forEach((item) => {
        if (bkxshd25s[item]) {
            bkxshd25Arr.push(bkxshd25s[item])
        }
    })
          console.log(`共${bkxshd25Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd25Arr.length; k++) {
                $.message = ""
                bkxshd25 = bkxshd25Arr[k]
                $.index = k + 1;
