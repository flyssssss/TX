...
APP:必看小说
变量 如
bkxshd44='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd44');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd44Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd44= $.isNode() ? (process.env.bkxshd44 ? process.env.bkxshd44 : "") : ($.getdata('bkxshd44') ? $.getdata('bkxshd44') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd44Arr.push($.getdata('bkxshd44'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd44Arr.push($.getdata(`bkxshd44${i}`))
            }
    console.log(`------------- 共${bkxshd44Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd44Arr.length; i++) {
        if (bkxshd44Arr[i]) {
          bkxshd44 = bkxshd44Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd44 && process.env.bkxshd44.indexOf('\n') > -1) {
            bkxshd44Arr = process.env.bkxshd44.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd44s = [process.env.bkxshd44]
        };
        Object.keys(bkxshd44s).forEach((item) => {
        if (bkxshd44s[item]) {
            bkxshd44Arr.push(bkxshd44s[item])
        }
    })
          console.log(`共${bkxshd44Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd44Arr.length; k++) {
                $.message = ""
                bkxshd44 = bkxshd44Arr[k]
                $.index = k + 1;
