...
APP:必看小说
变量 如
bkxshd27='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd27');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd27Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd27= $.isNode() ? (process.env.bkxshd27 ? process.env.bkxshd27 : "") : ($.getdata('bkxshd27') ? $.getdata('bkxshd27') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd27Arr.push($.getdata('bkxshd27'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd27Arr.push($.getdata(`bkxshd27${i}`))
            }
    console.log(`------------- 共${bkxshd27Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd27Arr.length; i++) {
        if (bkxshd27Arr[i]) {
          bkxshd27 = bkxshd27Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd27 && process.env.bkxshd27.indexOf('\n') > -1) {
            bkxshd27Arr = process.env.bkxshd27.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd27s = [process.env.bkxshd27]
        };
        Object.keys(bkxshd27s).forEach((item) => {
        if (bkxshd27s[item]) {
            bkxshd27Arr.push(bkxshd27s[item])
        }
    })
          console.log(`共${bkxshd27Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd27Arr.length; k++) {
                $.message = ""
                bkxshd27 = bkxshd27Arr[k]
                $.index = k + 1;
