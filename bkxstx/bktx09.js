...
APP:必看小说
变量 如
bkxshd9='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd09');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd9Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd9= $.isNode() ? (process.env.bkxshd9 ? process.env.bkxshd9 : "") : ($.getdata('bkxshd9') ? $.getdata('bkxshd9') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd9Arr.push($.getdata('bkxshd9'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd9Arr.push($.getdata(`bkxshd9${i}`))
            }
    console.log(`------------- 共${bkxshd9Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd9Arr.length; i++) {
        if (bkxshd9Arr[i]) {
          bkxshd9 = bkxshd9Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd9 && process.env.bkxshd9.indexOf('\n') > -1) {
            bkxshd9Arr = process.env.bkxshd9.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd9s = [process.env.bkxshd9]
        };
        Object.keys(bkxshd9s).forEach((item) => {
        if (bkxshd9s[item]) {
            bkxshd9Arr.push(bkxshd9s[item])
        }
    })
          console.log(`共${bkxshd9Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd9Arr.length; k++) {
                $.message = ""
                bkxshd9 = bkxshd9Arr[k]
                $.index = k + 1;
