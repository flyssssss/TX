...
APP:必看小说
变量 如
bkxshd37='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd37');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd37Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd37= $.isNode() ? (process.env.bkxshd37 ? process.env.bkxshd37 : "") : ($.getdata('bkxshd37') ? $.getdata('bkxshd37') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd37Arr.push($.getdata('bkxshd37'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd37Arr.push($.getdata(`bkxshd37${i}`))
            }
    console.log(`------------- 共${bkxshd37Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd37Arr.length; i++) {
        if (bkxshd37Arr[i]) {
          bkxshd37 = bkxshd37Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd37 && process.env.bkxshd37.indexOf('\n') > -1) {
            bkxshd37Arr = process.env.bkxshd37.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd37s = [process.env.bkxshd37]
        };
        Object.keys(bkxshd37s).forEach((item) => {
        if (bkxshd37s[item]) {
            bkxshd37Arr.push(bkxshd37s[item])
        }
    })
          console.log(`共${bkxshd37Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd37Arr.length; k++) {
                $.message = ""
                bkxshd37 = bkxshd37Arr[k]
                $.index = k + 1;
