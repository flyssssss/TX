...
const $ = new Env('bkxshd20');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd20Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd20= $.isNode() ? (process.env.bkxshd20 ? process.env.bkxshd20 : "") : ($.getdata('bkxshd20') ? $.getdata('bkxshd20') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd20Arr.push($.getdata('bkxshd20'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd20Arr.push($.getdata(`bkxshd20${i}`))
            }
    console.log(`------------- 共${bkxshd20Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd20Arr.length; i++) {
        if (bkxshd20Arr[i]) {
          bkxshd20 = bkxshd20Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd20 && process.env.bkxshd20.indexOf('\n') > -1) {
            bkxshd20Arr = process.env.bkxshd20.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd20s = [process.env.bkxshd20]
        };
        Object.keys(bkxshd20s).forEach((item) => {
        if (bkxshd20s[item]) {
            bkxshd20Arr.push(bkxshd20s[item])
        }
    })
          console.log(`共${bkxshd20Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd20Arr.length; k++) {
                $.message = ""
                bkxshd20 = bkxshd20Arr[k]
                $.index = k + 1;
