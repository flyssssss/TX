...
APP:必看小说
变量 如
bkxshd50='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd50');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd50Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd50= $.isNode() ? (process.env.bkxshd50 ? process.env.bkxshd50 : "") : ($.getdata('bkxshd50') ? $.getdata('bkxshd50') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd50Arr.push($.getdata('bkxshd50'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd50Arr.push($.getdata(`bkxshd50${i}`))
            }
    console.log(`------------- 共${bkxshd50Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd50Arr.length; i++) {
        if (bkxshd50Arr[i]) {
          bkxshd50 = bkxshd50Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd50 && process.env.bkxshd50.indexOf('\n') > -1) {
            bkxshd50Arr = process.env.bkxshd50.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd50s = [process.env.bkxshd50]
        };
        Object.keys(bkxshd50s).forEach((item) => {
        if (bkxshd50s[item]) {
            bkxshd50Arr.push(bkxshd50s[item])
        }
    })
          console.log(`共${bkxshd50Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd50Arr.length; k++) {
                $.message = ""
                bkxshd50 = bkxshd50Arr[k]
                $.index = k + 1;
