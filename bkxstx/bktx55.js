...
APP:必看小说
变量 如
bkxshd55='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd55');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd55Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd55= $.isNode() ? (process.env.bkxshd55 ? process.env.bkxshd55 : "") : ($.getdata('bkxshd55') ? $.getdata('bkxshd55') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd55Arr.push($.getdata('bkxshd55'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd55Arr.push($.getdata(`bkxshd55${i}`))
            }
    console.log(`------------- 共${bkxshd55Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd55Arr.length; i++) {
        if (bkxshd55Arr[i]) {
          bkxshd55 = bkxshd55Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd55 && process.env.bkxshd55.indexOf('\n') > -1) {
            bkxshd55Arr = process.env.bkxshd55.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd55s = [process.env.bkxshd55]
        };
        Object.keys(bkxshd55s).forEach((item) => {
        if (bkxshd55s[item]) {
            bkxshd55Arr.push(bkxshd55s[item])
        }
    })
          console.log(`共${bkxshd55Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd55Arr.length; k++) {
                $.message = ""
                bkxshd55 = bkxshd55Arr[k]
                $.index = k + 1;
