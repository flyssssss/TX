...
APP:必看小说
变量 如
bkxshd52='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd52');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd52Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd52= $.isNode() ? (process.env.bkxshd52 ? process.env.bkxshd52 : "") : ($.getdata('bkxshd52') ? $.getdata('bkxshd52') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd52Arr.push($.getdata('bkxshd52'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd52Arr.push($.getdata(`bkxshd52${i}`))
            }
    console.log(`------------- 共${bkxshd52Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd52Arr.length; i++) {
        if (bkxshd52Arr[i]) {
          bkxshd52 = bkxshd52Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd52 && process.env.bkxshd52.indexOf('\n') > -1) {
            bkxshd52Arr = process.env.bkxshd52.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd52s = [process.env.bkxshd52]
        };
        Object.keys(bkxshd52s).forEach((item) => {
        if (bkxshd52s[item]) {
            bkxshd52Arr.push(bkxshd52s[item])
        }
    })
          console.log(`共${bkxshd52Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd52Arr.length; k++) {
                $.message = ""
                bkxshd52 = bkxshd52Arr[k]
                $.index = k + 1;
