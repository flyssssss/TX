...
APP:必看小说
变量 如
bkxshd16='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd16');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd16Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd16= $.isNode() ? (process.env.bkxshd16 ? process.env.bkxshd16 : "") : ($.getdata('bkxshd16') ? $.getdata('bkxshd16') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd16Arr.push($.getdata('bkxshd16'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd16Arr.push($.getdata(`bkxshd16${i}`))
            }
    console.log(`------------- 共${bkxshd16Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd16Arr.length; i++) {
        if (bkxshd16Arr[i]) {
          bkxshd16 = bkxshd16Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd16 && process.env.bkxshd16.indexOf('\n') > -1) {
            bkxshd16Arr = process.env.bkxshd16.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd16s = [process.env.bkxshd16]
        };
        Object.keys(bkxshd16s).forEach((item) => {
        if (bkxshd16s[item]) {
            bkxshd16Arr.push(bkxshd16s[item])
        }
    })
          console.log(`共${bkxshd16Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd16Arr.length; k++) {
                $.message = ""
                bkxshd16 = bkxshd16Arr[k]
                $.index = k + 1;
