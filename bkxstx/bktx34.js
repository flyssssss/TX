...
APP:必看小说
变量 如
bkxshd34='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd34');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd34Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd34= $.isNode() ? (process.env.bkxshd34 ? process.env.bkxshd34 : "") : ($.getdata('bkxshd34') ? $.getdata('bkxshd34') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd34Arr.push($.getdata('bkxshd34'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd34Arr.push($.getdata(`bkxshd34${i}`))
            }
    console.log(`------------- 共${bkxshd34Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd34Arr.length; i++) {
        if (bkxshd34Arr[i]) {
          bkxshd34 = bkxshd34Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd34 && process.env.bkxshd34.indexOf('\n') > -1) {
            bkxshd34Arr = process.env.bkxshd34.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd34s = [process.env.bkxshd34]
        };
        Object.keys(bkxshd34s).forEach((item) => {
        if (bkxshd34s[item]) {
            bkxshd34Arr.push(bkxshd34s[item])
        }
    })
          console.log(`共${bkxshd34Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd34Arr.length; k++) {
                $.message = ""
                bkxshd34 = bkxshd34Arr[k]
                $.index = k + 1;
