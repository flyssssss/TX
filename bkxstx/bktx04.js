...
APP:必看小说
变量 如
bkxshd4='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd04');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd4Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd4= $.isNode() ? (process.env.bkxshd4 ? process.env.bkxshd4 : "") : ($.getdata('bkxshd4') ? $.getdata('bkxshd4') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd4Arr.push($.getdata('bkxshd4'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd4Arr.push($.getdata(`bkxshd4${i}`))
            }
    console.log(`------------- 共${bkxshd4Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd4Arr.length; i++) {
        if (bkxshd4Arr[i]) {
          bkxshd4 = bkxshd4Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd4 && process.env.bkxshd4.indexOf('\n') > -1) {
            bkxshd4Arr = process.env.bkxshd4.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd4s = [process.env.bkxshd4]
        };
        Object.keys(bkxshd4s).forEach((item) => {
        if (bkxshd4s[item]) {
            bkxshd4Arr.push(bkxshd4s[item])
        }
    })
          console.log(`共${bkxshd4Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd4Arr.length; k++) {
                $.message = ""
                bkxshd4 = bkxshd4Arr[k]
                $.index = k + 1;
