...
APP:必看小说
变量 如
bkxshd46='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd46');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd46Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd46= $.isNode() ? (process.env.bkxshd46 ? process.env.bkxshd46 : "") : ($.getdata('bkxshd46') ? $.getdata('bkxshd46') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd46Arr.push($.getdata('bkxshd46'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd46Arr.push($.getdata(`bkxshd46${i}`))
            }
    console.log(`------------- 共${bkxshd46Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd46Arr.length; i++) {
        if (bkxshd46Arr[i]) {
          bkxshd46 = bkxshd46Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd46 && process.env.bkxshd46.indexOf('\n') > -1) {
            bkxshd46Arr = process.env.bkxshd46.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd46s = [process.env.bkxshd46]
        };
        Object.keys(bkxshd46s).forEach((item) => {
        if (bkxshd46s[item]) {
            bkxshd46Arr.push(bkxshd46s[item])
        }
    })
          console.log(`共${bkxshd46Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd46Arr.length; k++) {
                $.message = ""
                bkxshd46 = bkxshd46Arr[k]
                $.index = k + 1;
