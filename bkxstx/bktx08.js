...
APP:必看小说
变量 如
bkxshd8='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd08');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd8Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd8= $.isNode() ? (process.env.bkxshd8 ? process.env.bkxshd8 : "") : ($.getdata('bkxshd8') ? $.getdata('bkxshd8') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd8Arr.push($.getdata('bkxshd8'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd8Arr.push($.getdata(`bkxshd8${i}`))
            }
    console.log(`------------- 共${bkxshd8Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd8Arr.length; i++) {
        if (bkxshd8Arr[i]) {
          bkxshd8 = bkxshd8Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd8 && process.env.bkxshd8.indexOf('\n') > -1) {
            bkxshd8Arr = process.env.bkxshd8.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd8s = [process.env.bkxshd8]
        };
        Object.keys(bkxshd8s).forEach((item) => {
        if (bkxshd8s[item]) {
            bkxshd8Arr.push(bkxshd8s[item])
        }
    })
          console.log(`共${bkxshd8Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd8Arr.length; k++) {
                $.message = ""
                bkxshd8 = bkxshd8Arr[k]
                $.index = k + 1;
