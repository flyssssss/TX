...
APP:必看小说
变量 如
bkxshd7='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd07');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd7Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd7= $.isNode() ? (process.env.bkxshd7 ? process.env.bkxshd7 : "") : ($.getdata('bkxshd7') ? $.getdata('bkxshd7') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd7Arr.push($.getdata('bkxshd7'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd7Arr.push($.getdata(`bkxshd7${i}`))
            }
    console.log(`------------- 共${bkxshd7Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd7Arr.length; i++) {
        if (bkxshd7Arr[i]) {
          bkxshd7 = bkxshd7Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd7 && process.env.bkxshd7.indexOf('\n') > -1) {
            bkxshd7Arr = process.env.bkxshd7.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd7s = [process.env.bkxshd7]
        };
        Object.keys(bkxshd7s).forEach((item) => {
        if (bkxshd7s[item]) {
            bkxshd7Arr.push(bkxshd7s[item])
        }
    })
          console.log(`共${bkxshd7Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd7Arr.length; k++) {
                $.message = ""
                bkxshd7 = bkxshd7Arr[k]
                $.index = k + 1;
