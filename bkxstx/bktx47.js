...
APP:必看小说
变量 如
bkxshd47='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd47');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd47Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd47= $.isNode() ? (process.env.bkxshd47 ? process.env.bkxshd47 : "") : ($.getdata('bkxshd47') ? $.getdata('bkxshd47') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd47Arr.push($.getdata('bkxshd47'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd47Arr.push($.getdata(`bkxshd47${i}`))
            }
    console.log(`------------- 共${bkxshd47Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd47Arr.length; i++) {
        if (bkxshd47Arr[i]) {
          bkxshd47 = bkxshd47Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd47 && process.env.bkxshd47.indexOf('\n') > -1) {
            bkxshd47Arr = process.env.bkxshd47.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd47s = [process.env.bkxshd47]
        };
        Object.keys(bkxshd47s).forEach((item) => {
        if (bkxshd47s[item]) {
            bkxshd47Arr.push(bkxshd47s[item])
        }
    })
          console.log(`共${bkxshd47Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd47Arr.length; k++) {
                $.message = ""
                bkxshd47 = bkxshd47Arr[k]
                $.index = k + 1;
