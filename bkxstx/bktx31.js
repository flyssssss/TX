...
APP:必看小说
变量 如
bkxshd31='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd31');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd31Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd31= $.isNode() ? (process.env.bkxshd31 ? process.env.bkxshd31 : "") : ($.getdata('bkxshd31') ? $.getdata('bkxshd31') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd31Arr.push($.getdata('bkxshd31'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd31Arr.push($.getdata(`bkxshd31${i}`))
            }
    console.log(`------------- 共${bkxshd31Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd31Arr.length; i++) {
        if (bkxshd31Arr[i]) {
          bkxshd31 = bkxshd31Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd31 && process.env.bkxshd31.indexOf('\n') > -1) {
            bkxshd31Arr = process.env.bkxshd31.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd31s = [process.env.bkxshd31]
        };
        Object.keys(bkxshd31s).forEach((item) => {
        if (bkxshd31s[item]) {
            bkxshd31Arr.push(bkxshd31s[item])
        }
    })
          console.log(`共${bkxshd31Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd31Arr.length; k++) {
                $.message = ""
                bkxshd31 = bkxshd31Arr[k]
                $.index = k + 1;
