...
APP:必看小说
变量 如
bkxshd22='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd22');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd22Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd22= $.isNode() ? (process.env.bkxshd22 ? process.env.bkxshd22 : "") : ($.getdata('bkxshd22') ? $.getdata('bkxshd22') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd22Arr.push($.getdata('bkxshd22'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd22Arr.push($.getdata(`bkxshd22${i}`))
            }
    console.log(`------------- 共${bkxshd22Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd22Arr.length; i++) {
        if (bkxshd22Arr[i]) {
          bkxshd22 = bkxshd22Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd22 && process.env.bkxshd22.indexOf('\n') > -1) {
            bkxshd22Arr = process.env.bkxshd22.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd22s = [process.env.bkxshd22]
        };
        Object.keys(bkxshd22s).forEach((item) => {
        if (bkxshd22s[item]) {
            bkxshd22Arr.push(bkxshd22s[item])
        }
    })
          console.log(`共${bkxshd22Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd22Arr.length; k++) {
                $.message = ""
                bkxshd22 = bkxshd22Arr[k]
                $.index = k + 1;
