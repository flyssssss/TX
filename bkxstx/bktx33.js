...
APP:必看小说
变量 如
bkxshd33='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd33');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd33Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd33= $.isNode() ? (process.env.bkxshd33 ? process.env.bkxshd33 : "") : ($.getdata('bkxshd33') ? $.getdata('bkxshd33') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd33Arr.push($.getdata('bkxshd33'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd33Arr.push($.getdata(`bkxshd33${i}`))
            }
    console.log(`------------- 共${bkxshd33Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd33Arr.length; i++) {
        if (bkxshd33Arr[i]) {
          bkxshd33 = bkxshd33Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd33 && process.env.bkxshd33.indexOf('\n') > -1) {
            bkxshd33Arr = process.env.bkxshd33.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd33s = [process.env.bkxshd33]
        };
        Object.keys(bkxshd33s).forEach((item) => {
        if (bkxshd33s[item]) {
            bkxshd33Arr.push(bkxshd33s[item])
        }
    })
          console.log(`共${bkxshd33Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd33Arr.length; k++) {
                $.message = ""
                bkxshd33 = bkxshd33Arr[k]
                $.index = k + 1;
