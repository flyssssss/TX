...
APP:必看小说
变量 如
bkxshd39='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd39');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd39Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd39= $.isNode() ? (process.env.bkxshd39 ? process.env.bkxshd39 : "") : ($.getdata('bkxshd39') ? $.getdata('bkxshd39') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd39Arr.push($.getdata('bkxshd39'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd39Arr.push($.getdata(`bkxshd39${i}`))
            }
    console.log(`------------- 共${bkxshd39Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd39Arr.length; i++) {
        if (bkxshd39Arr[i]) {
          bkxshd39 = bkxshd39Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd39 && process.env.bkxshd39.indexOf('\n') > -1) {
            bkxshd39Arr = process.env.bkxshd39.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd39s = [process.env.bkxshd39]
        };
        Object.keys(bkxshd39s).forEach((item) => {
        if (bkxshd39s[item]) {
            bkxshd39Arr.push(bkxshd39s[item])
        }
    })
          console.log(`共${bkxshd39Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd39Arr.length; k++) {
                $.message = ""
                bkxshd39 = bkxshd39Arr[k]
                $.index = k + 1;
