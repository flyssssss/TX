...
APP:必看小说
变量 如
bkxshd41='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd41');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd41Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd41= $.isNode() ? (process.env.bkxshd41 ? process.env.bkxshd41 : "") : ($.getdata('bkxshd41') ? $.getdata('bkxshd41') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd41Arr.push($.getdata('bkxshd41'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd41Arr.push($.getdata(`bkxshd41${i}`))
            }
    console.log(`------------- 共${bkxshd41Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd41Arr.length; i++) {
        if (bkxshd41Arr[i]) {
          bkxshd41 = bkxshd41Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd41 && process.env.bkxshd41.indexOf('\n') > -1) {
            bkxshd41Arr = process.env.bkxshd41.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd41s = [process.env.bkxshd41]
        };
        Object.keys(bkxshd41s).forEach((item) => {
        if (bkxshd41s[item]) {
            bkxshd41Arr.push(bkxshd41s[item])
        }
    })
          console.log(`共${bkxshd41Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd41Arr.length; k++) {
                $.message = ""
                bkxshd41 = bkxshd41Arr[k]
                $.index = k + 1;
