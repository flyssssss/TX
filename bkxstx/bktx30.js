...
APP:必看小说
变量 如
bkxshd30='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd30');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd30Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd30= $.isNode() ? (process.env.bkxshd30 ? process.env.bkxshd30 : "") : ($.getdata('bkxshd30') ? $.getdata('bkxshd30') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd30Arr.push($.getdata('bkxshd30'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd30Arr.push($.getdata(`bkxshd30${i}`))
            }
    console.log(`------------- 共${bkxshd30Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd30Arr.length; i++) {
        if (bkxshd30Arr[i]) {
          bkxshd30 = bkxshd30Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd30 && process.env.bkxshd30.indexOf('\n') > -1) {
            bkxshd30Arr = process.env.bkxshd30.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd30s = [process.env.bkxshd30]
        };
        Object.keys(bkxshd30s).forEach((item) => {
        if (bkxshd30s[item]) {
            bkxshd30Arr.push(bkxshd30s[item])
        }
    })
          console.log(`共${bkxshd30Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd30Arr.length; k++) {
                $.message = ""
                bkxshd30 = bkxshd30Arr[k]
                $.index = k + 1;
