...
APP:必看小说
变量 如
bkxshd10='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd10');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd10Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd10= $.isNode() ? (process.env.bkxshd10 ? process.env.bkxshd10 : "") : ($.getdata('bkxshd10') ? $.getdata('bkxshd10') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd10Arr.push($.getdata('bkxshd10'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd10Arr.push($.getdata(`bkxshd10${i}`))
            }
    console.log(`------------- 共${bkxshd10Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd10Arr.length; i++) {
        if (bkxshd10Arr[i]) {
          bkxshd10 = bkxshd10Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd10 && process.env.bkxshd10.indexOf('\n') > -1) {
            bkxshd10Arr = process.env.bkxshd10.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd10s = [process.env.bkxshd10]
        };
        Object.keys(bkxshd10s).forEach((item) => {
        if (bkxshd10s[item]) {
            bkxshd10Arr.push(bkxshd10s[item])
        }
    })
          console.log(`共${bkxshd10Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd10Arr.length; k++) {
                $.message = ""
                bkxshd10 = bkxshd10Arr[k]
                $.index = k + 1;
