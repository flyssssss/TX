...
APP:必看小说
变量 如
bkxshd36='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd36');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd36Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd36= $.isNode() ? (process.env.bkxshd36 ? process.env.bkxshd36 : "") : ($.getdata('bkxshd36') ? $.getdata('bkxshd36') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd36Arr.push($.getdata('bkxshd36'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd36Arr.push($.getdata(`bkxshd36${i}`))
            }
    console.log(`------------- 共${bkxshd36Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd36Arr.length; i++) {
        if (bkxshd36Arr[i]) {
          bkxshd36 = bkxshd36Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd36 && process.env.bkxshd36.indexOf('\n') > -1) {
            bkxshd36Arr = process.env.bkxshd36.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd36s = [process.env.bkxshd36]
        };
        Object.keys(bkxshd36s).forEach((item) => {
        if (bkxshd36s[item]) {
            bkxshd36Arr.push(bkxshd36s[item])
        }
    })
          console.log(`共${bkxshd36Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd36Arr.length; k++) {
                $.message = ""
                bkxshd36 = bkxshd36Arr[k]
                $.index = k + 1;
