...
APP:必看小说
变量 如
bkxshd6='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd06');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd6Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd6= $.isNode() ? (process.env.bkxshd6 ? process.env.bkxshd6 : "") : ($.getdata('bkxshd6') ? $.getdata('bkxshd6') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd6Arr.push($.getdata('bkxshd6'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd6Arr.push($.getdata(`bkxshd6${i}`))
            }
    console.log(`------------- 共${bkxshd6Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd6Arr.length; i++) {
        if (bkxshd6Arr[i]) {
          bkxshd6 = bkxshd6Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd6 && process.env.bkxshd6.indexOf('\n') > -1) {
            bkxshd6Arr = process.env.bkxshd6.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd6s = [process.env.bkxshd6]
        };
        Object.keys(bkxshd6s).forEach((item) => {
        if (bkxshd6s[item]) {
            bkxshd6Arr.push(bkxshd6s[item])
        }
    })
          console.log(`共${bkxshd6Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd6Arr.length; k++) {
                $.message = ""
                bkxshd6 = bkxshd6Arr[k]
                $.index = k + 1;
