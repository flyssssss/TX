...
APP:必看小说
变量 如
bkxshd43='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd43');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd43Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd43= $.isNode() ? (process.env.bkxshd43 ? process.env.bkxshd43 : "") : ($.getdata('bkxshd43') ? $.getdata('bkxshd43') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd43Arr.push($.getdata('bkxshd43'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd43Arr.push($.getdata(`bkxshd43${i}`))
            }
    console.log(`------------- 共${bkxshd43Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd43Arr.length; i++) {
        if (bkxshd43Arr[i]) {
          bkxshd43 = bkxshd43Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd43 && process.env.bkxshd43.indexOf('\n') > -1) {
            bkxshd43Arr = process.env.bkxshd43.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd43s = [process.env.bkxshd43]
        };
        Object.keys(bkxshd43s).forEach((item) => {
        if (bkxshd43s[item]) {
            bkxshd43Arr.push(bkxshd43s[item])
        }
    })
          console.log(`共${bkxshd43Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd43Arr.length; k++) {
                $.message = ""
                bkxshd43 = bkxshd43Arr[k]
                $.index = k + 1;
