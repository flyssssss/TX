...
APP:必看小说
变量 如
bkxshd5='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd05');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd5Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd5= $.isNode() ? (process.env.bkxshd5 ? process.env.bkxshd5 : "") : ($.getdata('bkxshd5') ? $.getdata('bkxshd5') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd5Arr.push($.getdata('bkxshd5'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd5Arr.push($.getdata(`bkxshd5${i}`))
            }
    console.log(`------------- 共${bkxshd5Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd5Arr.length; i++) {
        if (bkxshd5Arr[i]) {
          bkxshd5 = bkxshd5Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd5 && process.env.bkxshd5.indexOf('\n') > -1) {
            bkxshd5Arr = process.env.bkxshd5.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd5s = [process.env.bkxshd5]
        };
        Object.keys(bkxshd5s).forEach((item) => {
        if (bkxshd5s[item]) {
            bkxshd5Arr.push(bkxshd5s[item])
        }
    })
          console.log(`共${bkxshd5Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd5Arr.length; k++) {
                $.message = ""
                bkxshd5 = bkxshd5Arr[k]
                $.index = k + 1;
