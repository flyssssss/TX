...
APP:必看小说
变量 如
bkxshd59='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd59');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd59Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd59= $.isNode() ? (process.env.bkxshd59 ? process.env.bkxshd59 : "") : ($.getdata('bkxshd59') ? $.getdata('bkxshd59') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd59Arr.push($.getdata('bkxshd59'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd59Arr.push($.getdata(`bkxshd59${i}`))
            }
    console.log(`------------- 共${bkxshd59Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd59Arr.length; i++) {
        if (bkxshd59Arr[i]) {
          bkxshd59 = bkxshd59Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd59 && process.env.bkxshd59.indexOf('\n') > -1) {
            bkxshd59Arr = process.env.bkxshd59.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd59s = [process.env.bkxshd59]
        };
        Object.keys(bkxshd59s).forEach((item) => {
        if (bkxshd59s[item]) {
            bkxshd59Arr.push(bkxshd59s[item])
        }
    })
          console.log(`共${bkxshd59Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd59Arr.length; k++) {
                $.message = ""
                bkxshd59 = bkxshd59Arr[k]
                $.index = k + 1;
