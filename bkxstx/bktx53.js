...
APP:必看小说
变量 如
bkxshd53='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd53');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd53Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd53= $.isNode() ? (process.env.bkxshd53 ? process.env.bkxshd53 : "") : ($.getdata('bkxshd53') ? $.getdata('bkxshd53') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd53Arr.push($.getdata('bkxshd53'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd53Arr.push($.getdata(`bkxshd53${i}`))
            }
    console.log(`------------- 共${bkxshd53Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd53Arr.length; i++) {
        if (bkxshd53Arr[i]) {
          bkxshd53 = bkxshd53Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd53 && process.env.bkxshd53.indexOf('\n') > -1) {
            bkxshd53Arr = process.env.bkxshd53.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd53s = [process.env.bkxshd53]
        };
        Object.keys(bkxshd53s).forEach((item) => {
        if (bkxshd53s[item]) {
            bkxshd53Arr.push(bkxshd53s[item])
        }
    })
          console.log(`共${bkxshd53Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd53Arr.length; k++) {
                $.message = ""
                bkxshd53 = bkxshd53Arr[k]
                $.index = k + 1;
