...
APP:必看小说
变量 如
bkxshd51='sessionid=6e52cab4cefa43c193f3a466b0c33111'
抓包点我的 关键字 info
...
const $ = new Env('bkxshd51');
var crypto = require("crypto");
var xxxx=0;
let status;
status = (status = ($.getval("bkxsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let bkxshd51Arr = [],bkxscount = ''
const notify = $.isNode() ? require('./sendNotify') : '';
let bkxshd51= $.isNode() ? (process.env.bkxshd51 ? process.env.bkxshd51 : "") : ($.getdata('bkxshd51') ? $.getdata('bkxshd51') : "")
...
!(async () => {
      ...
      if(!$.isNode()){
          bkxshd51Arr.push($.getdata('bkxshd51'))
          let bkxscount = ($.getval('bkxscount') || '1');
          for (let i = 2; i <= bkxscount; i++) {
            bkxshd51Arr.push($.getdata(`bkxshd51${i}`))
            }
    console.log(`------------- 共${bkxshd51Arr.length}个账号-------------\n`)
      for (let i = 0; i < bkxshd51Arr.length; i++) {
        if (bkxshd51Arr[i]) {
          bkxshd51 = bkxshd51Arr[i];
          $.index = i + 1;

  }
}
      }else  {
          if (process.env.bkxshd51 && process.env.bkxshd51.indexOf('\n') > -1) {
            bkxshd51Arr = process.env.bkxshd51.split('\n');
            console.log(`您选择的是用"换行"隔开\n`)
        } else {
            bkxshd51s = [process.env.bkxshd51]
        };
        Object.keys(bkxshd51s).forEach((item) => {
        if (bkxshd51s[item]) {
            bkxshd51Arr.push(bkxshd51s[item])
        }
    })
          console.log(`共${bkxshd51Arr.length}个cookie`)
 for (let n = 0; n < 100;n++) {
          if (xxxx==1) break;         
	        for (let k = 0; k < bkxshd51Arr.length; k++) {
                $.message = ""
                bkxshd51 = bkxshd51Arr[k]
                $.index = k + 1;
